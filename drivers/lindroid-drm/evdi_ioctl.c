...
#if IS_ENABLED(CONFIG_DRM_LINDROID_EVDI_REAL_HWC)
int evdi_queue_power_event(struct evdi_device *evdi,
			   int display_id,
			   bool pwr_on)
{
	struct evdi_event *event;
	...
	event = evdi_event_alloc(evdi, disp_pwr,
				 atomic_inc_return(&evdi->events.next_poll_id),
				 &d_pwr, sizeof(d_pwr), NULL);
	if (!event)
		return -ENOMEM;
